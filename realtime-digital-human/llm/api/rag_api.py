...
def initialize_settings():
    """初始化全局设置"""
    Settings.llm = Ollama(model="xinhe_2025_0710:latest", temperature=0)
...
async def warmup_model(chat_engine: ContextChatEngine):
    ...
    while retries < max_retries:
        try:
            logger.info("开始预热模型...")
            test_input = "你好"
            await chat_engine.achat(test_input)
            logger.info("模型预热完成")
            break
        except Exception as e:
                ...
                async for text_chunk in response.async_response_gen():
                    ...
                    if (len(buffer) >= 10
                            or any(punc in buffer
                                   for punc in ("。", "！", "？", "\n")) or
                        (current_time - last_send_time) >= 0.5):  # 最大500ms延迟
                        if buffer.strip():
                            yield buffer
                            buffer = ""
