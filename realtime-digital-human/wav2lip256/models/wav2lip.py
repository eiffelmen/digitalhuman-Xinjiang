...
class Wav2Lip(nn.Module):
    ...
    def forward(self, audio_sequences, face_sequences, a_alpha=1.):
        ...
        if input_dim_size > 4:
            audio_sequences = torch.cat([audio_sequences[:, i] for i in range(
                audio_sequences.size(1))], dim=0)  # [bz, 5, 1, 80, 16]->[bz*5, 1, 80, 16]
            face_sequences = torch.cat([face_sequences[:, :, i] for i in range(
                face_sequences.size(2))], dim=0)  # [bz, 6, 5, 256, 256]->[bz*5, 6, 256, 256]
...
class Wav2Lip_disc_qual(nn.Module):
    def __init__(self):
        ...
        self.face_encoder_blocks = nn.ModuleList([
            nn.Sequential(nonorm_Conv2d(
                3, 32, kernel_size=7, stride=1, padding=3)),
    ...
    def perceptual_forward(self, false_face_sequences):  # 前传生成图像
        # [bz, 3, 5, 256, 256]->[bz*5, 3, 256, 256]
        false_face_sequences = self.to_2d(false_face_sequences)
        # [bz*5, 3, 256, 256]->[bz*5, 3, 128, 256]
        false_face_sequences = self.get_lower_half(false_face_sequences)

        false_feats = false_face_sequences
        # [bz*5, 3, 128, 256]->[bz*5, 512, 1, 1]
        for f in self.face_encoder_blocks:
            false_feats = f(false_feats)

        # [bz*5, 512, 1, 1]->[bz*5, 1, 1]
