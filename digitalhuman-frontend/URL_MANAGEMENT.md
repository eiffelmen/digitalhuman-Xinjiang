...
// Electron环境: http://example.com:8000/api/user/info

// 调用后端服务器接口
const url = buildApiUrl('backend', '/human');
// 浏览器环境: /backend/human
// Electron环境: http://example.com:8010/human

// 调用 RAG 服务器接口
const url = buildApiUrl('rag', '/v1/rag');
// 浏览器环境: /v1/rag
// Electron环境: http://example.com:8888/v1/rag
```
...
// Electron环境: ws://example.com:8000/ws/asr

// 连接 LLM WebSocket
const llmUrl = buildWsUrl('llm', `/ws/${sessionId}`);
// 浏览器环境: ws://localhost:3000/llm/ws/xxx
// Electron环境: ws://example.com:8011/ws/xxx
```
...
|-----------|-----------|---------------|---------|
| main | example.com:8000 | /api | 主服务器，WebSocket、通用接口 |
| backend | example.com:8010 | /backend | 后端服务，数字人控制接口 |
| asr | example.com:10099 | /asr | 语音识别服务 |
| rag | example.com:8888 | (空字符串) | RAG知识库服务 |
...
const DEFAULT_BACKEND_CONFIG = {
  mainServer: 'example.com:8010',

  apiServers: {
    main: 'example.com:8000',
    backend: 'example.com:8010',
    // ... 其他配置 ...

    // 新增服务器配置
    newService: 'service.example.internal:9000',  // ✅ 添加这一行
  },
...
export default defineConfig({
  server: {
    proxy: {
      '/v1/upload_file': 'http://example.com:8888',
      '/v1/rag': 'http://example.com:8888',
      // ... 其他代理配置 ...

      // ✅ 添加新服务的代理配置
      '/new_service': {
        target: 'http://service.example.internal:9000',
        changeOrigin: true,
...
const DEFAULT_BACKEND_CONFIG = {
  apiServers: {
    main: 'example.com:8000',
    backend: 'example.com:8010',
    llm: 'example.com:8011',
    asr: 'example.com:10099',
    tts: 'example.com:7000',  // ✅ 新增
    // ... 其他配置
  },
};
...
|-----------|------|------------------|-------------|
| main | /user/info | `/api/user/info` | `http://example.com:8000/user/info` |
| backend | /human | `/backend/human` | `http://example.com:8010/human` |
| rag | /v1/rag | `/v1/rag` | `http://example.com:8888/v1/rag` |

### buildWsUrl 返回值对比

| serverKey | path | 浏览器环境（开发） | Electron环境 |
|-----------|------|------------------|-------------|
| main | /ws/asr | `ws://localhost:3000/ws/asr` | `ws://example.com:8000/ws/asr` |
| llm | /ws/chat | `ws://localhost:3000/llm/ws/chat` | `ws://example.com:8011/ws/chat` |
...
2. **避免硬编码**: 永远不要在代码中直接写服务器地址
   ```javascript
   // ❌ 错误示例
   fetch('http://example.com:8000/api/user')
