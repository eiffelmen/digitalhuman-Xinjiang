...
const MAIN_API_HOST = env.VITE_MAIN_API_HOST || BACKEND_HOST;
const RAG_HOST = env.VITE_RAG_HOST || BACKEND_HOST;
const ASR_HOST = env.VITE_ASR_HOST || BACKEND_HOST;

// 默认后端配置
const DEFAULT_BACKEND_CONFIG = {
  // 主后端服务器
  mainServer: `${BACKEND_HOST}:${env.VITE_BACKEND_PORT || '8010'}`,

  // API服务器映射
  apiServers: {
    main: `${MAIN_API_HOST}:${env.VITE_MAIN_API_PORT || '8000'}`,      // /api
    backend: `${BACKEND_HOST}:${env.VITE_BACKEND_PORT || '8010'}`,     // /backend
    asr: `${ASR_HOST}:${env.VITE_ASR_PORT || '10099'}`,                // /asr
    rag: `${RAG_HOST}:${env.VITE_RAG_PORT || '8888'}`,                 // /v1/rag, /v1/upload_file
  },
