...
const emits = defineEmits(['recordChange', 'close', 'asr-result']);
...
const { waitUntilSpeaking, waitUntilSilent } = useAvatarSpeechSync();
...
async function handleProcessingState(context) {
	...
	while(
		...
		if (await waitUntilSpeaking(stateMachineAbortController.signal)){
...
<template>
	<div
		...
		v-show="showChatLog"
		:style="`background-image: url('${isMultiModal ? getPublicUrl('/chat_bg_model.png') : getPublicUrl('/chat_bg_new.png')}');`"
	>
		<div
			ref="chatBox"
