...
const ASR_PORT = env.VITE_ASR_PORT || '10099';
...
export default defineConfig({
	...
	server: {
		...
		https: false,
		proxy: {
			'/v1/upload_file': `http://${RAG_HOST}:${RAG_PORT}`,

			'/v1/rag': `http://${RAG_HOST}:${RAG_PORT}`,
			'/asr': `ws://${ASR_HOST}:${ASR_PORT}`,
			'/backend/ws-audio': {
			...
			'/api': {
				target: `http://${MAIN_API_HOST}:${MAIN_API_PORT}`,
				changeOrigin: true,
